...
Each study is a standalone runner that trains its grid, scores every arm on
common evaluation paths, and appends one JSON line per run to its store with the
commit, library versions, device, seeds, configuration, and loss history. Tables
and figures regenerate from the stores alone. Runners resume from completed
record names, so an interrupted grid picks up where it stopped, and `--smoke`
redirects to a separate store with reduced sizes so trial runs never mask full
results.
...
The Heston studies accept `--fused` to sample with the fused CUDA kernel, which
generates two orders of magnitude faster but draws from its own Philox streams;
the committed stores were produced with the eager default and reproduce against
it.
...
Every record's `provenance` carries the commit hash that produced it, so a
stored number is checkable against the exact code that generated it.
