...
@given(
    self=st.builds(
        ...
        .flatmap(lambda count: st.lists(st.floats(-3.0, 3.0), min_size=count, max_size=count))
        .map(tuple),
    ),
    state=st.integers(0, 2**31).map(
        lambda seed: GBMSimulator(s0=100.0, sigma=0.2, maturity=1.0, n_steps=_STEPS).simulate(
            _PATHS, noise=NoiseSpec(seed=seed)
        )
    ),
    t=st.integers(0, _STEPS - 1),
    tau=st.floats(0.0, 1.0).map(torch.tensor),
    position=st.lists(st.floats(-2.0, 2.0), min_size=_PATHS, max_size=_PATHS).map(torch.tensor),
)
