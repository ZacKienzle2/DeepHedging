...
@given(
    problem=st.builds(
        ...
        generator=st.one_of(
            st.just(ZeroGenerator()),
            st.builds(DiscountGenerator, rate=st.floats(0.0, 0.1)),
        ),
    ...
    config=st.builds(
        ...
        hidden_sizes=st.one_of(
            st.just((8, 8)), st.lists(st.integers(1, 8), max_size=2).map(tuple)
        ),
