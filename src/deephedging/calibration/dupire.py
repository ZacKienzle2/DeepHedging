...
calibration and simulation. The variance is clamped to a wide admissible
band because finite differences near the wings divide one small number
by another, and an unguarded ratio there turns a tiny butterfly
violation into an absurd volatility.
"""
...
def dupire_surface(
    ...
    inner_span = (taus[2:] - taus[:-2]).unsqueeze(1)
    time_derivative[1:-1] = (prices[2:] - prices[:-2]) / inner_span
